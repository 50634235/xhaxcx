...
class MemberController extends BaseController
{
     ...
     * @var string
     */
    protected $get_access_token_url = 'https://api.weixin.qq.com/sns/jscode2session';

    public function login()
    {
        // $addon_config = get_addon_config('SyncLogin');
        // echo json_encode($addon_config);
        // exit;
        $code = I('code');
        ...
        $res = $this->http($this->get_access_token_url, $params, 'POST');
        $res = json_decode($res, true);
        // $res = json_decode($res);
        $openid = $res['openid'];
        $session_key = $res['session_key'];
        // $res格式
        // {"session_key":"PBNp7w4jGNxOpP0R+1yxcA==","expires_in":7200,"openid":"o1MoD0cmaJB_OTb1FvQHA7_d1BF0"}
        ...
        $user_info = json_decode($decrypt_data,true);
        //
        if (empty($user_info['openId'])) {
            $this->jsonReturn(['参数错误']);
        }

        $session = array('session_key' => $session_key, 'type' => 'wxxcx', 'openid' => $openid, 'user_info' => $user_info);

        session('synclogin', $session);
        // 重新获取session
        $this->getSession(); 
        // 此处没有增加绑定过程,直接走注册新账号,如果后期需要此需求,可在此添加
        $this->unBind();
    }
    ...
    public function unBind()
    {
        ...
        $data = $this->user_info;
        // $user_info = D('Addons://SyncLogin/Info')->weixin($token);
        $user_info['type'] = 'wxxcx';
        $user_info['name'] = $data['nickName'];
        $user_info['nick'] = $data['nickName'];
        $user_info['head'] = $data['avatarUrl'];
        $user_info['sex'] = $data['gender']=='1'? 0:1;
     ...
     * @DateTime 2017-11-03T11:35:22+0800
    ...
    private function loginWithoutpwd($uid)
    {
        if (0 < $uid) { //UC登陆成功
            ...
            if ($this->doLogin($uid)) { //登陆用户
                ...
                $err_msg = $Member->getError();
                $this->jsonReturn(null, $err_msg, 1);
            }
        }
    }
    ...
    {
        $rs = D('Wxxcx/Member')->login($uid, $remember); //登陆
        if ($rs) {
     ...
     * 获取session_key 接口
     * @Author   王衍生                      50634235@qq.com
     * @DateTime 2017-11-01T17:42:05+0800
     * @return   [type]                   [description]
     */
